...
async fn main() {
    ...
    let lambda_4 = {
        let script = std::fs::read("./examples/script-test/test4_1-0_o.js").unwrap();
        let blob = client.blob().new(script).await.unwrap();
        blob.into_lambda("test4_1-0_o").await.unwrap()
    };
    let lambda_5 = {
        let script = std::fs::read("./examples/script-test/test5_1-1_o.js").unwrap();
        let blob = client.blob().new(script).await.unwrap();
        blob.into_lambda("test5_1-1_o").await.unwrap()
    };
    let lambda_6 = {
        let script = std::fs::read("./examples/script-test/test6_0-0_o.js").unwrap();
        let blob = client.blob().new(script).await.unwrap();
        blob.into_lambda("test6_0-0_o").await.unwrap()
    };
    //
    // /////
