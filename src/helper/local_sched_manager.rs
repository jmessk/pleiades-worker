...
impl LocalSchedManager {
    ...
    pub fn view(&self) {
        ...
        for item in &self.list {
            ...
            let num_jobs = item.local_sched.num_jobs();
            ...
            println!("Local Scheduler {id}: \tnum_jobs: {num_jobs}");
        }
    ...
    pub fn shortest(&mut self) -> &mut local_sched::Controller {
        let item = self
            ...
            .min_by_key(|item| {
                let num_jobs = item.local_sched.num_jobs();
                tracing::trace!("Executor {}: num_jobs: {:?}", item.local_sched.id, num_jobs);
                num_jobs
            })
            .unwrap();
...
impl Builder {
    pub fn insert(&mut self, local_sched_controller: local_sched::Controller) {
        self.list.push(Item {
